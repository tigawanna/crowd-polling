...
interface AddBulkPollValuesProps {}

export function AddBulkPollValues({}: AddBulkPollValuesProps) {
  const { pb } = usePocketbase();
  const bulk_insert_mutation = useMutation({
    mutationFn: () => {
      pb.autoCancellation(false);
      const bulkInsertPromise = words.map((item) => {
        return pb.from("rendercon_crowd_polls").create({ value: item.key });
      });
      return Promise.allSettled(bulkInsertPromise);
    },
    meta: {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      <SpinnerButton
        variant={"outline"}
        mutation={bulk_insert_mutation}
        onClick={() => bulk_insert_mutation.mutate()}
      >
        Bulk insert
      </SpinnerButton>
    </div>
